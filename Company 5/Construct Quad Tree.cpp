...
public:
    Node* rec_construct(vector<vector<int>>& grid,int row_start,int row_end,int col_start,int col_end,int factor)
    {
        ...
        Node* ans;
        if(sum==0) // If all the cells are filled by 0
            ans=new Node(false,true);
        else if(sum==(factor*factor)) // If all the cells are filled by 1
            ans=new Node(true,true);
        else // If cells are mix of 0 and 1 thus we apply recursion in this case
        {
